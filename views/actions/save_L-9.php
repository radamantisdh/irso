...
$propie_ciudad		= mysql_real_escape_string(stripslashes($_POST['propie_ciudad']));
...
elseif(empty($fecha_ingreso)) { echo $error."El campo <b>Fecha de Ingreso</b> no puede quedar vacio".$finerror; $cago = true; }
	...
	if($cago == false) {
            ...
            $frase3 = str_replace($numeros, $regiones, $propie_region);
            $frase4 = str_replace("_", " ", $propie_ciudad);

            $frase5 = str_replace($numeros, $regiones, $rep_leg_region);
            $frase6 = str_replace("_", " ", $rep_leg_ciudad);
    ...
    VALUES ('$nro_asd', '$actividad_rubro', '$estab_calle', '$estab_numero', 
            '$estab_resto', '$estab_telefono', '$frase1', '$frase2', '$propie_nombre', 
            '$propie_ap_pat', '$propie_ap_mat', '$propie_rut', '$propie_dv', '$propie_calle', 
            '$propie_numero', '$propie_resto', '$frase3', '$frase4', '$rep_leg_nombre', 
            '$rep_leg_ap_pat', '$rep_leg_ap_mat', '$rep_leg_rut', '$rep_leg_dv', '$rep_leg_calle', 
            '$rep_leg_numero', '$rep_leg_resto', '$frase5', '$frase6', 
            '$rep_leg_telefono', '$rep_leg_email', '$fecha_ingreso')";
	pg_query("$sql");
